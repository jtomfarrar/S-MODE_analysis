...
def plot_ops_area_IOP1(ax,**kwargs):
...
def plot_ops_area_pilot(ax,**kwargs):
...
def chl_map_SMODE(url,zoom,V,time_window):
    ...
    cb.set_label('SST [$\circ$C]',fontsize = 10)
    #plot_ops_area(ax,transform=ccrs.PlateCarree(),color='k')
    plot_ops_area_IOP1(ax,transform=ccrs.PlateCarree(),color='k')
    # Create an inset GeoAxes showing the location of the Solomon Islands.
    ...
    xs, ys = zip(*coord) #create lists of x and y values
    sub_ax.plot(xs,ys, transform=ccrs.PlateCarree(), linewidth=2)
    plot_ops_area_IOP1(sub_ax,transform=ccrs.PlateCarree(),color='k')
...
def download_THREDDS(server_url, request_url,download_path=None):
    ...
    doanload_path : str
        Path to folder where downloaded files should be placed
    ...
    for f in files:
        ...
        file_name = download_path + file_prefix  + '.nc'
