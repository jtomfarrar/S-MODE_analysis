...
# %%
os.system('cd ~/Python/S-MODE_analysis/code_IOP2')
...
__figdir__ = '../plots/IOP2_summary3/'
...
def plot_SST_map(ds2, mean_SST):
    ...
    V = [mean_SST+i for i in [-1,1]] #range for colorbar
...
request_url = 'catalog/IOP2_2023/satellite/' + ds_name + '/catalog.xml'
...
good_files=[]
skip = 1
for n in range(30,len(sst_filelist)):
    ds = xr.open_dataset(sst_filelist[n])
    day_str = ds.time.dt.strftime("%Y-%m-%d-%H%MZ").values
    # print(day_str[0])
    if ds.time<np.datetime64('2023-03-26T09') or ds.time>np.datetime64('2023-05-03T09'):
        continue
